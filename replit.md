...
- `GOOGLE_PLACES_API_KEY` is required for venue photo carousels and place-ID lookups (`fetchGooglePlacesPhotos`, `findGooglePlaceIdByName`). Without it, carousels are skipped and place IDs can't be auto-discovered. Set this secret before running the real NYC population pass. The venue candidate-sourcing step (`listVenueCandidates`) uses the Google Places Text Search API via this key.
